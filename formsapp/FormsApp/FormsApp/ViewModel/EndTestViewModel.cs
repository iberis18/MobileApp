...
namespace FormsApp.ViewModel
{
    ...
    internal class EndTestViewModel : INotifyPropertyChanged
    {
        ...
        private void ResultsProcessing()
        {
            ...
            //сопоставление с нормами (расшифровка)
        ...
        {
            Result result = App.Database.GetUserResultByTestId(userId, test.Id);
            if (result == null)
                App.Database.SaveResult(new Result() {Scores = sum, Text = comment, TestId = test.Id, UserId = userId });
            else
            {
                result.Scores = sum;
                result.Text = comment;
